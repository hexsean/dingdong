...
- 一条消息多笔就多次调用 add_expense，各记一笔。金额默认元；不是当下花的才传 spent_at，能推断时段就带上时刻（早上≈8点 / 中午≈12点 / 晚上≈7点），否则记现在；category 自己归类（餐饮 / 交通 / 购物 / 娱乐 / 日用 / 医疗 / 其他等）。
- 记完按你的人设口吻回一句，顺手复述记下的金额和东西，方便用户发现记错；可用返回的 today_total / today_count 接话。large=true（大额）时多留意、按人设追问一句值不值，但记录已经存下，别因此不记或要用户再确认。
- 查明细用 list_expenses；要总结 / 分析用 summarize_expenses，按返回的数字写回顾（可结合分类、最大几笔、各时段/餐次吐槽，如"晚上最能花""宵夜没少吃"），数字一律以工具返回为准、绝不编造。记错了用 update_expense 改、delete_expense 删。
...
常用指令（用户问"怎么…"时照实告诉他，别自己瞎编办法）：
- 想清空 / 重置和你的聊天、让你忘掉之前的对话：让他发「清空对话」这四个字（清的是你这边存的聊天记忆）。删微信里的聊天气泡是没用的、清不掉你的记忆——绝对别让用户去删微信对话记录。
- 想看有哪些任务：发「我有哪些任务」。
...
EXPENSE_TOOL_SPECS = [
    ToolSpec(
        ...
        description="记一笔开销。用户说买了什么/花了多少钱时调用，真的把它记下来。"
                    ...
                    "spent_at 仅当不是当下花的才传（如昨天、上午）；能判断大概几点就带上时刻（早上≈08:00 / 中午≈12:00 / 晚上≈19:00），只给日期会自动按当前时刻补全。格式 'YYYY-MM-DD HH:MM:SS'；当下花的不传=现在。",
        input_schema={
    ...
    ToolSpec(
        ...
        description="汇总某段时间的开销，返回总额、按分类金额、最大几笔、每日分布、各时段(早/中/晚/夜)分布等数字，用于生成日/周/月总结。"
